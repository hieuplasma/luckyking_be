...
export class KenoSocketService {
  ...
  setEmployeeIsBusy(client: Socket) {
    const staffMember = this.clients.find(
  ...
  sendKenoLottery(lotteries: Lottery[]) {
    this.server.emit(socketEvent.NewKenoLottery, {
      lotteries: serializeBigInt(lotteries),
    });
  }

  printLottery(lotteryId: string) {
    const lottery = this.pendingLotteries.find(
      (lottery) => lottery.id === lotteryId,
    );
    if (lottery) {
  ...
  confirmLottery(lotteryId: string) {
    this.pendingLotteries = this.pendingLotteries.filter(
      (lottery) => lottery.id !== lotteryId,
    );
  }
  ...
  async sendKenoToStaff() {
    ...
    for (const lottery of this.pendingLotteries) {
      ...
      const { kenoDuration, kenoPauseTime, stopDistributingKenoTickets } =
        config;
      ...
      const staffMemberToSend = this.clients.find(
      ...
      this.server
        .to(socketId)
        .emit(socketEvent.NewKenoLottery, {
          lotteries: serializeBigInt([lottery]),
          kenoDuration,
        });
    ...
    if (returnedLotteryIds.length > 0) {
      this.pendingLotteries = this.pendingLotteries.filter(
        (lottery) => !returnedLotteryIds.includes(lottery.id),
      );
    }
  }
}
